...
public static class GmailAccess
{
    static string[] Scopes = {GmailService.Scope.MailGoogleCom};
    static string ApplicationName = "Kursovaya";

    public static GmailService GetService()
    {
        UserCredential credential;
        using (FileStream stream = new FileStream(Environment.CurrentDirectory + "\\oauth_client_credentials.json",
                   FileMode.Open, FileAccess.Read))
        {
            String FilePath = Path.Combine(Environment.CurrentDirectory, "APITokenCredentials");
        ...
        try
        {
            ...
            foreach (MessagePart part in parts)
            {
                if (!String.IsNullOrEmpty(part.Filename))
                {
                    ...
                    MessagePartBody attachPart = GServices.Users.Messages.Attachments.Get(userId,
                        messageId, attId).Execute();
